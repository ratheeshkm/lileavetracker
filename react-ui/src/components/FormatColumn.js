...
function formatColumn(
  ...
  const data =
    (formatExtraData &&
      formatExtraData.filter((item) => item.rowId === row.id)) ||
    [];
  const fieldNameOrg = fieldName || 'name';
  if (fieldSearchArray.length) {
    cellValue = fieldSearchArray.filter((item) => item.id === cellValue)[0][
      fieldNameOrg
    ];
  }
  if (data.length) {
    ...
    if (rowId === row.id) {
      ...
      if (fieldSearchArray && oldValue) {
        oldValue = fieldSearchArray.filter((item) => item.id === oldValue)[0][
          fieldNameOrg
        ];
      }
